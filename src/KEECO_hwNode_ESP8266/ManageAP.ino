...
void initWifiOnBoot() {
  ...
  WiFi.mode(WIFI_STA);
  stationGotIPHandler = WiFi.onStationModeGotIP(&stopAPSrvCallback);
  stationDisconnectedHandler = WiFi.onStationModeDisconnected(&startAPSrvCallback);
  ...
  if (wifiConnWait < 60) {
    ...
    Serial.println("");
    Serial.println("Warning: Could not connect to WiFi network!");
    startAPServices();
  }
  start_mDNS();
}
